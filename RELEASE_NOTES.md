...
- **`install --dry-run` stopped printing your host config.** It used to dump the whole file twice, before and after the edit. On a real machine that is 5751 lines carrying the API keys and connection strings of every *other* MCP server you have configured — and the natural thing to do with dry-run output is paste it into a bug report. The preview now shows the `skillforge` entry and a line confirming everything else is preserved. `--show-full-config` brings the old behaviour back for anyone who wants it, and states what it exposes.
- **`--entry bin` actually works on Windows now.** The first cut probed the command without a shell, reasoning that this is how a host spawns it. On Windows that can never succeed: a bare name gives `ENOENT`, and Node refuses to run a `.cmd` shim without a shell on purpose (the BatBadBut mitigation, CVE-2024-27980). The tell was that `npx` fails the identical probe while working perfectly as a registered MCP command — hosts resolve npm shims themselves. The probe now finds the command on `PATH` honouring `PATHEXT` and probes that exact file, using a shell only for a resolved Windows shim.

**Engineering snapshot**

- 1003 tests passing + 2 skipped. The regression guard (`tests/integration/lifecycle-exit.test.ts`) spawns the real built server, performs an MCP handshake, closes the transport, and fails if the process outlives it — plus a second case that kills the parent while stdin stays open.
